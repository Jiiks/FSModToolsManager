...
internal partial class FormBase : Form {
    ...
    protected override void WndProc(ref Message m) {
        ...
        if (m.Msg == WM_NCHITTEST) {
            ...
            if (res == HTCLIENT) {
                if(TopMovesInstead && clientPoint.Y < CaptionSize && clientPoint.X > ResizeBorderSize && clientPoint.X < ClientSize.Width - ResizeBorderSize) {
                    m.Result = HTCAPTION;
                    return;
                }
    ...
    public int CaptionSize { get; set; } = 50;
    ...
    public Color CaptionColour { get; set; } = Color.Black;
